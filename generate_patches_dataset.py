...
def patches_from_superpixels(orig_path, output_filename, nsuperpixels, patch_size):
    ...
    for i in range(nimages):
        ...
        finalniters = ift.IGraphISF_Mean(igraph, seeds, 0.1, 12, 2)

        S: ift.Set = ift.SuperpixelCenterSetFromIGraph(igraph)

        S_np: np.ndarray = S.AsNumPy(input_orig[i])

        invalid_voxel = 0

        img_np:np.ndarray = input_orig[i].AsNumPy().reshape((input[i].n, input[i].m))

        for j in range(nsuperpixels):
            p = int(S_np[j])
            ...
            for k in range(Q.n):
                ...
                if ift.MValidVoxel(input[i], v) == 1:
                    q = ift.MGetVoxelIndex(input[i], v)
                    sample_feats.append(img_np[q])
                else:
                    invalid_voxel = 1
                    break

            if invalid_voxel == 0:
                patches_csv_file.write("{}-{:03d}-{:03d}-{:03d}-{:03d}-{:010d}\n".format(img_basename, u.x, u.y, patch_size, s))
                ids.append(s)
                truelabels.append(truelabel)
                ref_data.append(os.path.abspath(fs[i]))
                samples.append(np.ndarray(sample_feats).tolist())
                s += 1


    y = np.ndarray(truelabels, dtype=np.int32)
    X = np.ndarray(samples, dtype=np.float32)
